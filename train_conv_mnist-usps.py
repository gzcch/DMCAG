...
class SingleViewModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pretrain:
            x_bar = self.decoder(self.delatent(z).view(-1, 64, 3, 3))
            return x_bar, z
...
def pretrain_aes():
   ...
   # np.savetxt('./Mu rawf.csv', f_all.cpu().detach().numpy(), fmt='%.2f', delimiter=' ')
   # np.savetxt('./Mu rawz.csv', z_all.cpu().detach().numpy(), fmt='%.2f', delimiter=' ')
...
def fineTuning():
    ...
    #wp = (torch.ones((1, args.viewNumber)) / args.viewNumber).cuda()
    ...
    for epoch in range(1000):
        ...
        for viewIndex in range(args.viewNumber):
            ...
            #cmin = calculate_c(output[viewIndex][1].T, model.aes[viewIndex].A_weight.data,gamma=args.gamma)
            #model.aes[viewIndex].c_weight.data = cmin
            z_temp=output[viewIndex][1]
        ...
        if epoch<=500:
            ...
            #cmin = calculate_c(z_all.T, model.Al_weight.data, gamma=args.gamma)
            #f_temp=cacluate_U(cmin)
        ...
        #kl_loss= nn.KLDivLoss(reduction='mean')(torch.tensor(0.05).log(), output[viewIndex][1])
        view_loss=0*view_loss
...
if __name__ == "__main__":
    ...
    if args.dataset == 'MNIST_USPS':
        args.n_input = [[1,28,28], [1,28,28] ]
        args.viewNumber = 2
        args.instanceNumber =5000
        args.batch_size = 5000
        args.n_clusters = 10
        args.arch = 50
        args.gamma= 1
        args.save_path = './data/MNIST_USPS.pkl'
        setup_seed(3)
